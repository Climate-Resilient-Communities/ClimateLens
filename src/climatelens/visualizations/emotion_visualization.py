...
REQUIRED_COLUMNS = {"twitter": ["created_at", "text"], "reddit": ["body", "created_utc"]}
...
def load_environment() -> Dict[str, Path]:
    """
    Thin compatibility wrapper over :func:`utils.runtime.load_runtime`.

    Returns a dict with ``data_dir`` (where to read emotion-classified CSVs,
    preferring the output_data_dir produced by emotion_classification.py)
    and ``output_dir`` (where visualizations should be written).
    """
    runtime = load_runtime()
    log.info("emotion visualizations starting")
    log.info("paths:\n%s", runtime.describe())

    # Prefer the emotion-classified output; fall back to raw data dir.
    data_dir = runtime.output_data_dir
    if not any(data_dir.glob("*.csv")):
        log.warning(
            "no CSVs in %s; falling back to %s. Did you run emotion_classification.py first?",
            data_dir,
            runtime.data_dir,
        )
    ...
    return {
        "data_dir": data_dir,
        "output_dir": runtime.output_vis_dir,
    }
...
def find_csv_files(data_dir: Path) -> Dict[str, Path]:
    ...
    # Find Twitter file
    twitter_file = next((f for f in csv_files if "twitter" in f.name.lower()), None)
    if twitter_file:
    ...
    if reddit_file:
        ...
        missing_files.append(
            'Reddit (filename should contain "reddit", "anticonsumption", "comments", or "submissions")'
        )
...
def load_emotion_model():
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        EMOTION_MODEL_NAME, use_safetensors=True
    )
...
def detect_emotions_batch(
    texts: List[str], emotion_classifier, batch_size: int = 32
) -> pd.DataFrame:
    """
...
def add_emotions_to_datasets(
    datasets: Dict[str, pd.DataFrame], emotion_classifier
) -> Dict[str, pd.DataFrame]:
    """
    Run emotion detection on all datasets and add results as new columns.

    If the input dataframe already carries ``emotion_label`` (because it came
    from ``emotion_classification.py`` upstream), the classifier is skipped
    and the existing labels are reused. This avoids running two inconsistent
    emotion models back-to-back.
    """
    updated_datasets = {}
    for name, df in datasets.items():
        log.info("processing %s (%d rows)", name, len(df))

        if "emotion_label" in df.columns:
            log.info("  emotion_label already present; reusing existing labels")
            updated_datasets[name] = df
            dist = df["emotion_label"].value_counts()
            for emotion, count in dist.items():
                pct = count / len(df) * 100
                log.info("    %s: %d (%.1f%%)", emotion, count, pct)
            continue

        text_col = next((c for c in ("body", "text") if c in df.columns), None)
        if text_col is None:
            log.warning("skipping %s: no text column found", name)
            continue
...
def create_emotion_wordcloud(
    texts: List[str], emotion: str, dataset_name: str
) -> Optional[WordCloud]:
    """
...
def save_wordcloud(wordcloud: WordCloud, title: str, filepath: Path) -> None:
    """
...
def prepare_time_column(df: pd.DataFrame, dataset_name: str) -> Tuple[pd.DataFrame, str]:
    """
...
def create_emotion_timeseries(df: pd.DataFrame, dataset_name: str, output_dir: Path) -> List[str]:
    """
    ...
    emotion_counts = (
        df_plot.groupby(["time_period", "emotion_label"]).size().reset_index(name="count")
    )
    ...
    fig_pct.update_layout(
        ...
        legend=dict(title="Emotions", orientation="v", yanchor="top", y=1, xanchor="left", x=1.02),
...
def create_summary_report(datasets: Dict[str, pd.DataFrame], output_dir: Path) -> str:
    """
...
def run_pipeline() -> None:
    ...
    # Step 3: Load emotion model only if any dataset is missing labels.
    needs_classification = any("emotion_label" not in df.columns for df in datasets.values())
    if needs_classification:
        log.info("[3/7] loading emotion detection model")
        emotion_classifier, device_name = load_emotion_model()
        log.info("  model loaded on %s", device_name)
    else:
        log.info("[3/7] all datasets already carry emotion_label; skipping model load")
        emotion_classifier = None

    # Step 4: Detect emotions
    log.info("[4/7] detecting emotions")
    datasets = add_emotions_to_datasets(datasets, emotion_classifier)
    log.info("  emotion detection complete")
