...
def hierarchy(
    ...
    dendrogram(
        linkage_matrix, labels=[f"Doc {i + 1}" for i in range(n_samples)]
...
reddit_vectorizer = CountVectorizer(
    ngram_range=(1, 1), stop_words=stopwords, max_df=0.25, min_df=10
)
...
for i in range(len(names)):
    ...
    plt.text(
        coherence[i], perplexity[i], f"{names[i]}", fontsize=10, ha="right", va="bottom"
    )  # names next to points
...
)  # move legend so it's not on plot
